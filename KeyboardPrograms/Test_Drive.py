"""
This program provides manual control for a QCar using the Pygame library. It enables the user to 
control the car's throttle and steering via keyboard inputs while displaying a graphical window. 

Key Features:
- Uses Pygame for event handling to capture keyboard inputs.
- Implements smooth acceleration and deceleration mechanics.
- Supports both forward and reverse driving modes.
- Adjusts LED indicators based on steering direction, headlights, and braking.
- Runs two separate threads: one for driving control and another for camera preview.
- Ensures safe program termination by handling keyboard interrupts.
